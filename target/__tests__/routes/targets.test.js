const Target = require('../../models/target')

describe('Get Targets', () => {
  const expected = {
    description: 'Welcome to us',
    image: 'data:image/png;base64,',
    place: 'Geldrop',
    lat: 51.4107812,
    long: 5.5583965
  }

  it('Create user without userId', async () => {
    const target = new Target({
      ...expected
    })

    const error = target.validateSync()
    expect(error.errors.userId.message).toEqual('Path `userId` is required.')
  })

  it('Create target', async () => {
    const target = new Target({
      userId: '6251857383fd40fe5f3e1ac0',
      ...expected
    })

    const error = target.validateSync()
    expect(error).toBeUndefined()
  })

  it('Create target without image', async () => {
    const target = new Target({
      userId: '6251857383fd40fe5f3e1ac0',
      ...expected
    })

    const error = target.validateSync()
    expect(error).toBeUndefined()
  })

  it('should validate error on invalid base64 image', function () {
    const target = new Target({
      ...expected,
      image: 'dete:hoi/doei;base32,iVBORw0KGgoAAAANSXEaY4Yk4wxWcY5V/92QRKvP3UNz8bWk1qMxjGTOMThZ3kvgLI5AzFfo379UAAAAASUVORK5CYII='
    })

    const error = target.validateSync()
    expect(error.errors.image).toBeDefined()
  })

  it('should validate error on emtpy submission', function () {
    const target = new Target()

    const error = target.validateSync()
    expect(error.errors.description).toBeDefined()
    expect(error.errors.image).toBeDefined()
    expect(error.errors.tags).toBeUndefined()
    expect(error.errors.ratings).toBeUndefined()
    expect(error.errors.place).toBeDefined()
    expect(error.errors.lat).toBeDefined()
    expect(error.errors.long).toBeDefined()
    expect(error.errors.userId).toBeDefined()
  })
})
